...
//LANG CONTROL
$mydbcolumnsstr = "cer_title_id > ? AND cer_title_active = ?";
$mydbarray = [0,1];
$resultcer = $dbcon->dbslca("cer_title",$mydbcolumnsstr,$mydbarray);

//TAKE MENU DATA
$mydbcolumnsstr = "cer_id > ? AND cer_lang = ?";
$mydbarray = [0,$langnumaber];
$result = $dbcon->dbslca("cer",$mydbcolumnsstr,$mydbarray);

$myreturndata = [];
foreach ($resultLang as $key2 => $value2) {
	foreach ($resultcer as $key => $value) {
		if($value["cer_title_lang"] == $value2["lang_id"]){
			$myreturndata[$value2["lang_id"]]["cer_title_id"] = $value["cer_title_id"];
			$myreturndata[$value2["lang_id"]]["cer_title_name"] = $value["cer_title_name"];
			$myreturndata[$value2["lang_id"]]["cer_title_uniqid"] = $value["cer_title_uniqid"];
			$myreturndata[$value2["lang_id"]]["cer_title_lang"] = $value["cer_title_lang"];
		}
	}
}
					...
					<div class="main-content">
						...
						<div class="row gutters">
							<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12">
								<div class="card">
									<div class="card-header">Certificate Title</div>
										<div class="card-body">
										<form action="./db/cer_title.php" method="POST" enctype="multipart/form-data">
											<input type="hidden" name="id" value="<?php echo $resultcer[0]["cer_title_id"] ?>">
											...
											<div class="tab-pane fade show  <?php if($keylang == 0){ echo 'active'; } ?>" id="<?php echo $valuelang['lang_name'] ?>" role="tabpanel" aria-labelledby="<?php echo $valuelang['lang_name'] ?>-tab">
												<div class="form-row">
													<div class="form-group col-md-12">
														<label for="input1" class="col-form-label">Certificate Title</label>
														<input type="text" required value="<?php if(isset($myreturndata[$valuelang['lang_id']])){ echo $myreturndata[$valuelang['lang_id']]["cer_title_name"]; }else{} ?>" name="name-<?php echo $valuelang['lang_id'] ?>" class="form-control" id="input1" placeholder="Certificate Title">
													</div>
												</div>	
											</div>
										<?php }  ?>
										<button type="submit" class="btn btn-primary">Set Certificate Title</button>   
										</form>
										</div>
									</div>
								</div>
							...
							<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12">
								<div class="card">
									<div class="card-header">
										Certificate List
										<button type="button" class="btn  btn-success" style="float:right"  data-toggle="modal" data-target="#addModal"><i class="icon-plus"></i> New Certificate Item</button>
									</div>
									<div class="card-body">
									<table id="basicExample" class="table table-striped table-bordered">
										...
										<?php if($result == false){ }else{ foreach($result as $key => $value){ ?>
																<tr>
											<td><img src="./uploads/<?php echo $value['cer_image']; ?>" style="width:50px;" alt="<?php echo $value['cer_id']; ?>"></td>  
											<td>
												<?php if($value['cer_active'] == 1){?>
													<span class="badge badge-pill badge-success">Active</span>
												<?php }else{ ?>
													<span class="badge badge-pill badge-warning">Passive</span>
												<?php } ?>
											</td>
											<td><?php echo $value['cer_date']; ?></td>
											<td>
												<?php if($value['cer_active'] == 1){?>
												<button class="btn btn-warning btn-rounded"  data-toggle="modal" data-target="#pasModal" onclick="pasFunc('<?php echo $value["cer_id"]; ?>')" ><i class="icon-star" style="color:white"></i></button>
												<?php }else{ ?>
												<button class="btn btn-success btn-rounded"  data-toggle="modal" data-target="#activeModal" onclick="actFunc('<?php echo $value["cer_id"]; ?>')" ><i class="icon-star" style="color:white"></i></button>
												<?php } ?>
												<button class="btn btn-info btn-rounded"  data-toggle="modal" data-target="#cogModal" onclick="setFunc('<?php echo $value["cer_id"]; ?>')" ><i class="icon-settings" style="color:white"></i></button>
												<button class="btn btn-danger btn-rounded"  data-toggle="modal" data-target="#delModal" onclick="delFunc('<?php echo $value["cer_id"]; ?>')" ><i class="icon-trash" style="color:white"></i></button>
											</td>
																</tr>
										<?php } } ?>
										</tbody>
				...
				<div class="modal fade" id="addModal" tabindex="-1" role="dialog" aria-labelledby="addModalLabel" aria-hidden="true">
                      <div class="modal-dialog" role="document">
                          <div class="modal-content">
                              <form action="./db/addcer.php" method="POST"  enctype="multipart/form-data">
                              <div class="modal-header">
                                  <h5 class="modal-title" id="addModalLabel">New Certificate Form</h5>
                              ...
                              <div class="modal-body">
                                ...
                                <div class="tab-content" id="myTabContent">
                                  ...
                                  <div class="tab-pane fade show <?php if($keylang == 0){ echo 'active'; } ?>" id="add<?php echo $valuelang['lang_name'] ?>" role="tabpanel" aria-labelledby="<?php echo $valuelang['lang_name'] ?>-tab">
                                    <div class="form-row">
                                      <div class="form-group col-md-12">
                                        <label for="input1" class="col-form-label">Certificate Image</label>
                                        <hr>
                  ...
                  <div class="modal fade" id="cogModal" tabindex="-1" role="dialog" aria-labelledby="cogModalLabel" aria-hidden="true">
                      <div class="modal-dialog" role="document">
                          <div class="modal-content">
                          <form action="./db/setcer.php" method="POST"  enctype="multipart/form-data">
                              <div class="modal-header">
                                  <h5 class="modal-title" id="cogModalLabel">Set Certificate Form</h5>
                                  <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                      <span aria-hidden="true">&times;</span>
                                  </button>
                              </div>
                              ...
                              <div class="modal-footer">
                                  <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                                  <button type="submit" class="btn btn-primary">Set Certificate</button>
                              </div>
                          </form>
                          </div>
                      </div>
                  ...
                  <div class="modal fade" id="delModal" tabindex="-1" role="dialog" aria-labelledby="delModalLabel" aria-hidden="true">
                      <div class="modal-dialog" role="document">
                          <div class="modal-content">
                          <form action="./db/delcer.php" method="POST">
                              <div class="modal-header">
                                  <h5 class="modal-title" id="delModalLabel">Delete Certificate Form</h5>
                                  <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                      <span aria-hidden="true">&times;</span>
                                  </button>
                              </div>
                  ...
                  <div class="modal fade" id="pasModal" tabindex="-1" role="dialog" aria-labelledby="pasModalLabel" aria-hidden="true">
                      <div class="modal-dialog" role="document">
                          <div class="modal-content">
                          <form action="./db/statuscer.php" method="POST">
                              <div class="modal-header">
...
<script>
    ...
    function setFunc(id){
      $.post("./db/infocer.php",{ id: ""+id+"" }, function(data,status){ 
        obj = JSON.parse(data);
        var mysubstr ="file";
