...
(function ($) {
  ...
  // sticky header - start
  // --------------------------------------------------
  // Navbar made static: do not toggle sticky/floating behavior anymore.
  // (Scroll handlers left commented-out for reference; can be safely removed.)
  /*
  ...
  */
  ...
  $(document).on('input', '.slider-input', function () {
    ...
    const $container = $(this).closest('.comparison-slider');
    $container.find('.before-img').css('clip-path', `inset(0 ${100 - value}% 0 0)`);
    $container.find('.slider-handle').css('left', value + "%");
  });
